...
const planWithRetry = async (entry: BenchmarkCatalogEntry, deps: Awaited<ReturnType<typeof createDeps>> extends Result<infer T> ? T : never) => {
  ...
  for (let attempt = 0; !planned.ok && attempt < 2 && transientPlanError(planned.error.message); attempt += 1) {
    console.error(`[benchmark:${entry.fileStem}] planning retry ${attempt + 1}/2: ${planned.error.message}`);
...
export async function runBenchmarkSuite(entry: BenchmarkCatalogEntry, base: AgentloopConfig): Promise<Result<BenchmarkResult>> {
  ...
  let repoPath = '';
  try {
    const boot = await bootstrapBenchmarkRepo(entry); if (!boot.ok) { console.error(`[benchmark:${entry.fileStem}] bootstrap failed: ${boot.error.message}`); return ok(failed(entry, started, boot.error.message)); }
    repoPath = boot.value;
    ...
    const deps = await createDeps(config, false); if (!deps.ok) { console.error(`[benchmark:${entry.fileStem}] deps failed: ${deps.error.message}`); return ok(failed(entry, started, deps.error.message)); }
    const planned = await planWithRetry(entry, deps.value); if (!planned.ok) { console.error(`[benchmark:${entry.fileStem}] planning failed: ${planned.error.message}`); return ok(failed(entry, started, planned.error.message)); }
    const enqueued = await enqueuePlan(deps.value.queue, planned.value); if (!enqueued.ok) { console.error(`[benchmark:${entry.fileStem}] enqueue failed: ${enqueued.error.message}`); return ok(failed(entry, started, enqueued.error.message)); }
    ...
    if (!acceptance.ok) console.error(`[benchmark:${entry.fileStem}] acceptance failed: ${acceptance.error.message}`);
    if (!metrics.ok) console.error(`[benchmark:${entry.fileStem}] metrics failed: ${metrics.error.message}`);
    if (!tasks.ok) console.error(`[benchmark:${entry.fileStem}] queue failed: ${tasks.error.message}`);
    if (!orchestrated.ok) console.error(`[benchmark:${entry.fileStem}] orchestrator failed: ${orchestrated.error.message}`);
    if (tasks.ok) {
      ...
      console.error(`[benchmark:${entry.fileStem}] queue counts: ${JSON.stringify(counts)}`);
    }
    if (metrics.ok) console.error(`[benchmark:${entry.fileStem}] metrics count: ${metrics.value.length}`);
    const checks = [
    ...
    if (repoPath) {
      if (process.env.AGENTLOOP_KEEP_BENCHMARK_REPO === '1') console.error(`[benchmark:${entry.fileStem}] keeping repo: ${repoPath}`);
