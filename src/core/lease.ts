...
export async function withLease<T>(
  ...
  const timer = setInterval(() => {
    void renew()
      .then(r => { if (!r.ok && !leaseError) leaseError = err(r.error.code, r.error.message, renewalDetails(r.error.details)); })
      .catch(e => { if (!leaseError) leaseError = err('UNKNOWN', e instanceof Error ? e.message : 'Lease renewal threw', renewalDetails()); });
  }, intervalMs);
  try {
